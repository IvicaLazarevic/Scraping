...
__scriptName__ = 'cineplexx.rs.py'
__version__ = '0.1'
...
def cineplexx():
    URL = 'https://www.cineplexx.rs/filmovi/uskoro/'
    RES = requests.get(URL)
    SOUP = BeautifulSoup(RES.content, 'html.parser')
    for film in SOUP.findAll('div', {'class':'span3'}):
        print(f'[+] Uskoro u bioskopu: {film.find("h2").text}')
        print(f'[+] Link do filma: {film.find("h2").find("a")["href"][2:]}\n')

if __name__ == '__main__':
    print('\n[+] Dobrodosli u skriptu koja radi scraping sajta cineplexx.rs\n')
    cineplexx()