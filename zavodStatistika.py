...
__scriptName__ = 'zavodStatistika.py'
__version__ = '0.1'
...
def zavodZaStatistiku():
    URL = 'https://www.stat.gov.rs/RSS'
    RES = requests.get(URL)
    SOUP = BeautifulSoup(RES.content, 'xml')

    for vest in SOUP.findAll('item'):
        print(f'[+] Naziv vesti: {vest.find("title").text.strip()}')
...
if __name__ == '__main__':
    print('\n[+] Dobrodosli u skriptu koja radi scrap stat.gov.rs\n')
    zavodZaStatistiku()